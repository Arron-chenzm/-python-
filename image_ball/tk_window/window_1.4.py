...
def show():
     ...
     new1 = entry1.get()
     sig1.set(new1)
Button(fm2, text='START', height=3,command = show(),relief = SUNKEN).pack(side=BOTTOM, pady=20, fill=X, expand=NO)
fm2.grid(row=0, column=1, rowspan=2)
fm2.pack_propagate(0)

fm3 = Frame(window ,height = 180-80, width=1700,bg = 'red')
Button(fm3, text='A 55',font=('Arial', 15),padx = 10,relief = SUNKEN).pack(side=LEFT, fill=BOTH, expand=YES)
Button(fm3, text='B 30',font=('Arial', 15),padx = 10,relief = SUNKEN).pack(side=LEFT, fill=BOTH, expand=YES)
Button(fm3, text='C 22',font=('Arial', 15),padx = 10,relief = SUNKEN).pack(side=LEFT, fill=BOTH, expand=YES)
Button(fm3, text='D 41',font=('Arial', 15),padx = 10,relief = SUNKEN).pack(side=LEFT, fill=BOTH, expand=YES)
